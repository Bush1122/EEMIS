...
<div class="col">
<input type="text" name="marks[]" required class="form-control mb-2" placeholder="Obtained Marks">
</div>
</div>
<?php } ?>
...
if(isset($_POST['result'])){
	...
	$student_id=$_POST['id'];
	$marks=$_POST['marks'];
	$sql="insert into result values('','$subject','$class','$section')";
	$con->query($sql);
	$result_id = $con->insert_id;
	for($i=0;$i<count($student_id);$i++){
		$sql="insert into result_details values('','$result_id','".$student_id[$i]."','".$marks[$i]."')";
		$result=mysqli_query($con,$sql);
	}
	if($result){
		echo "<script>window.location.href='result.php'
		alert('Result Added Successfully')</script>";
		
	}
	else{
		echo "<script>window.location.href='add_result.php'
		alert('Sorry')</script>";
